...
void noise_generator (CC *h_noise, int N, double POWER){    
...
void input_field_T(CC *Ap, double *T, int SIZE, double T0, double POWER, char m){
    switch(m){
        ...
        case 'g' :
            ...
            for (int i = 0; i < SIZE; i++){
                Ap[i].x = sqrt(POWER) * exp(-0.5*T[i]*T[i]/(T0*T0));
                Ap[i].y = 0;
            }
            break;
        case 's' :
...
__global__ void equal(CC *auxp, CC *auxs, CC *Apw, CC* Asw, int SIZE){
	long int idx = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void add_GDD(CC *As, CC *aux, double *w, double GDD, int SIZE){
    ...
    if (idx < SIZE){
        ...
        aux[idx].y = As[idx].y * sin(0.5*w[idx]*w[idx]*GDD) + As[idx].x * cos(0.5*w[idx]*w[idx]*GDD);
	}
    if (idx < SIZE){
		As[idx].x = aux[idx].x;
		As[idx].y = aux[idx].y;
...
__global__ void write_signal(CC *As_total, CC *As, int nn, int N_ps, int extra_win, int N_rt){
