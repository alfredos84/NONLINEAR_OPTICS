...
R           = 0.3 # mirror reflectivity
dz          = Lc/100 # step size in z-crystal
...
P_threshold = spot*n_pump*eps0*c*(ls*n_signal/2/pi/deff/Lc)**2*((1-0.3)/0.3)
...
for delta in [-0.1,0.0,+0.1]:
    ...
    if(save_files is True):
        plt.savefig("Nanosecond_OPO_paper/spectrum_delta_{:1.1f}_pow_2Pth.png".format(delta))
    ...
    for be in [int(0.4*N_rt), int(0.8*N_rt)]:
        ...
        if(save_files is True):
            plt.savefig("Nanosecond_OPO_paper/phase_delta_{:1.1f}_rt_{:1d}pow_2Pth.png".format(delta,be))
